...
- **Don't flip `web.depends_on.ingestor.condition` from `service_started` to `service_healthy` in `docker-compose.yml`.** CI runs with `INGEST_MODE=smoke` (`.github/workflows/ci.yml`), the ingestor exits 0 before its healthcheck can mark it healthy, and `--wait` hangs until the CI timeout. The `service_started` + healthcheck combination gives operators visibility via `docker compose ps` without breaking CI. If you need a stricter production compose, use a prod-only overlay — don't change the base file.
