  ...
  try {
    ...
    const counts = await writeGraph(
      driver,
      {
        ...
        // PR 1 of #61: CID node + DWDM edge wiring lands in D4 (cron.ts /
        // index.ts source reads). Empty arrays here keep typecheck green
        // until then — writer phases are no-ops on empty input.
        cids: [],
        dwdm_edges: [],
      },
      resolverCfg,
